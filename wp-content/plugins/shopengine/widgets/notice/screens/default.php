...
if (\Elementor\Plugin::$instance->editor->is_edit_mode() || \Elementor\Plugin::$instance->preview->is_preview_mode()):
    ...
    if (!empty($page_type) && $page_type === 'single'): ?>
			<div class="woocommerce-message" role="alert">
				<button title="<?php esc_html_e('View Cart Item','shopengine')?>" tabindex="1" class="button wc-forward"><?php esc_html_e('View cart', 'shopengine')?></button><?php esc_html_e('“Beanie” has been added to your cart.', 'shopengine')?>
			</div>
		<?php else: ?>
