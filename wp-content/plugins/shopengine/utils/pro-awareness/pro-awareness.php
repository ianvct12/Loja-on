	...
	class Pro_Awareness
    {
		...
		public function set_page_grid($conf = []) {
			...
			if(!empty($conf['url'])) {
				...
				$this->grids[] = [
					'url'         => $conf['url'],
					'title'       => empty($conf['title']) ? esc_html__('Default Title', 'shopengine') : $conf['title'],
		...
		public function set_products( $product = [] ) {			
			$this->products[] = [
				...
				'title'       => empty( $product['title'] ) ? esc_html__( 'Default Title', 'shopengine' ) : $product['title'],
            ...
            <div class="pro_aware grid_container wpmet_pro_a-grid-container">
				...
				<div class="wpmet-products">
					<div class="wpmet-products__header">
						<h1><?php esc_html_e('Take your website to the next level','shopengine'); ?></h1>
						<p><?php esc_html_e('We have some plugins you can install to get most from Wordpress.','shopengine'); ?><br> <?php echo esc_html('These are absolute FREE to use.','elementskit-lite'); ?></p>
					</div>
					<div class="wpmet-products__content">
						<?php foreach ( $this->products as $product ) : ?>
							...
							<span><?php esc_html_e($product['description'],'shopengine'); ?></span></a>
						<?php endforeach; ?>
					</div>
				</div>
